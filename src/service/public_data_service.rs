...
use utoipa::ToSchema;
use crate::client::{CachingClient, PublicDataCachingClient};
...
impl PublicDataService {
    ...
    pub async fn create_public_data(&self, bytes: Bytes, evm_wallet: Wallet, store_type: StoreType) -> Result<Chunk, PublicDataError> {
        let data_address = PublicDataCachingClient::new(self.caching_client.clone()).data_put_public(bytes, PaymentOption::from(&evm_wallet), store_type).await?;
    ...
    pub async fn get_public_data_binary(&self, address: String) -> Result<Bytes, PublicDataError> {
        match DataAddress::from_hex(address.as_str()) {
            Ok(data_address) => PublicDataCachingClient::new(self.caching_client.clone()).data_get_public(&data_address).await,
