...
pub struct ChunkChannel {
    ...
    chunk_download_threads: usize,
}

impl ChunkChannel {
    pub fn new(id: String, data_map: DataMap, autonomi_client: Client, chunk_download_threads: usize) -> ChunkChannel {
        ChunkChannel { id, data_map, autonomi_client, chunk_download_threads }
    }
    
    pub fn open(&self, range_from: u64, range_to: u64) -> ChunkReceiver {
        let (sender, receiver) = channel(self.chunk_download_threads);
