@echo off
REM Script to fix the target installation issue and cross-compile AntTP on Windows

echo === AntTP Target Fix and Cross-Compilation Helper ===
echo This script will fix the target installation issue and cross-compile AntTP for Windows
echo.
...
REM Fix the Rust target installation
echo Fixing Rust target installation...

REM Remove the target if it exists
rustup target remove x86_64-pc-windows-gnu 2>nul

REM Update Rust
echo Updating Rust...
rustup update

REM Add the target with verbose output
echo Adding Windows target with verbose output...
rustup target add x86_64-pc-windows-gnu --verbose

REM Create .cargo directory and config.toml if they don't exist
...
if not exist %USERPROFILE%\.cargo\config.toml (
    ...
    echo linker = "x86_64-w64-mingw32-gcc" >> %USERPROFILE%\.cargo\config.toml
    echo ar = "x86_64-w64-mingw32-gcc-ar" >> %USERPROFILE%\.cargo\config.toml
)

REM Check if MinGW is installed and in PATH
where x86_64-w64-mingw32-gcc >nul 2>&1
if %ERRORLEVEL% neq 0 (
    echo MinGW GCC not found in PATH. Please install MinGW-w64 and add it to your PATH.
    echo You can download it from: https://www.mingw-w64.org/downloads/
    exit /b 1
)

echo Rust target installation fixed
...
set CXX_x86_64_pc_windows_gnu=x86_64-w64-mingw32-g++

REM Build the project with verbose output
echo Building for Windows with verbose output...
