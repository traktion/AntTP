...
impl FileService {
    ...
    pub async fn download_data_stream(
        ...
        let derived_range_to = if range_to == u64::MAX { file_size - 1 } else { range_to };
        ...
        if is_range_request {
            ...
            let response_range_to = derived_range_to - offset_modifier;
            info!("streaming item [{}] at addr [{}], range_from: [{}], range_to: [{}], offset_modifier: [{}], size_modifier: [{}], file_size: [{}], response_range_from: [{}], response_range_to: [{}]",
            path_str, format!("{:x}", xor_name), range_from, range_to, offset_modifier, size_modifier, file_size, response_range_from, response_range_to);
            Ok(HttpResponse::PartialContent()
                .insert_header(ContentRange(ContentRangeSpec::Bytes { range: Some((response_range_from, response_range_to)), instance_length: Some(file_size) }))
                .insert_header(cache_control_header)
                .insert_header(expires_header)
                .insert_header(etag_header)
                .insert_header(cors_allow_all)
            ...
            info!("streaming item [{}] at addr [{}], range_from: [{}], range_to: [{}], offset_modifier: [{}], size_modifier: [{}], file_size: [{}]",
            path_str, format!("{:x}", xor_name), range_from, range_to, offset_modifier, size_modifier,file_size);
            Ok(HttpResponse::Ok()
        ...
        if let Some(range) = request.headers().get(header::RANGE) {
            ...
            if let Some((range_from_str, range_to_str)) = range_value.split_once("-") {
                ...
                let range_to_override = match range_to_str.parse::<u64>() {
                    ...
                    Err(_) => {
                        if size_modifier != 0 {
                            offset_modifier + size_modifier - 1
                        } else {
                            u64::MAX
                        }
                    }
