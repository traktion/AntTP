#!/bin/bash
# Script to fix the target installation issue and cross-compile AntTP for Windows
...
echo -e "${BLUE}=== AntTP Target Fix and Cross-Compilation Helper ===${NC}"
echo -e "${BLUE}This script will fix the target installation issue and cross-compile AntTP for Windows${NC}"
echo ""
...
# Function to fix Rust target installation
fix_rust_target() {
    echo -e "${YELLOW}Fixing Rust target installation...${NC}"
    
    # Remove the target if it exists
    rustup target remove x86_64-pc-windows-gnu || true
    ...
    echo -e "${YELLOW}Configuring linker...${NC}"
    if ! grep -q "\[target.x86_64-pc-windows-gnu\]" ~/.cargo/config.toml; then
...
EOF
    ...
    echo -e "${GREEN}Rust target installation fixed${NC}"
}
...
# Fix the crunchy crate
fix_crunchy_crate

# Fix the Rust target installation
fix_rust_target

# Cross-compile directly using MinGW
cross_compile_direct
